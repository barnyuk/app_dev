...
wet, temperature, prev_prot_path = None, None, None

objects_list = []

def get_prev_prot_path(path:str) -> str:
    global prev_prot_path
    prev_prot_path = path

def get_temperature_and_wet(temp, w, prev):
    global wet, temperature, prev_prot_path
    wet = w
    temperature = temp
    prev_prot_path = prev
...
def find_prev_data(bsn_id: str):

    search_pattern = os.path.join(prev_prot_path, f"*{bsn_id}*.docx")
    protocols = [f for f in glob.glob(search_pattern) if not os.path.basename(f).startswith("~$")]

    if not protocols:
        return None

    for protocol in sorted(protocols, key=os.path.getctime, reverse=True):
        try:
            doc = docx.Document(protocol)

            if doc.tables:
                return deepcopy(doc.tables[-1]._tbl)
                
        except Exception as e:
           
            print(f"Ошибка при чтении файла {protocol}: {e}")
            continue
